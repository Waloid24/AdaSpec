...
def get_template(model_name):
    ...
    if model_name.startswith("Llama-3"):
        return get_conversation_template("llama-3")
...
def sample_specbench_requests(
    ...
    # Load the dataset.
    dataset = load_questions(dataset_path, 0, 480)
    ...
    while 1:
        ...
        prompt, prompt_token_ids = convert_question_to_input_ids(question, conv, tokenizer)
        prompt_len = len(prompt_token_ids)
        output_len = question["output_len"]
...
async def get_request(
    ...
    request_source = []
    if trace != "uniform":
        request_source = csv.reader(open(trace, 'r'))
        lines = list(request_source)
        _, request_source = lines[0], lines[1:]
    ...
    for idx, request in enumerate(input_requests):
        ...
        if trace == "uniform":
            ...
            current_ts = float(request_source[idx][0])
            interval = current_ts - start_ts
            start_ts = current_ts
...
async def benchmark(
    ...
    async for request in get_request(input_requests, trace, request_rate):
...
def main(args: argparse.Namespace):
    ...
    if args.dataset is not None:
        ...
        input_requests = sample_specbench_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            model_name=args.model.split("/")[-1],
            fixed_output_len=args.specbench_output_len,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["specbench", "sharegpt", "sonnet", "random", "hf", "cnn_dailymail"],
    ...
    specbench_group = parser.add_argument_group("specbench dataset options")
    specbench_group.add_argument(
        "--specbench-output-len",
        type=int,
        default=256,
        help=
