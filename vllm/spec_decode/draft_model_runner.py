...
class TP1DraftModelRunner(ModelRunner):
    ...
    def execute_model(
        ...
        if self.speculative_config.ssd or self.speculative_config.dynamic_spec:
        ...
        for step in range(num_steps):
            ...
            # Naive SpecInfer implementation with very pool performance due to the limitation of PageAttention
            specinfer_k = 3
            if step == 2 and self.speculative_config.specinfer:
                group_batch_size = logits.shape[0] // specinfer_k
                _, topk_indices = torch.topk(logits[0:group_batch_size], specinfer_k, dim=-1)
                for i in range(group_batch_size):
                    for j in range(1, specinfer_k):
                        logits[j * group_batch_size + i][topk_indices[i][j].item()] = \
                            logits[j * group_batch_size + i][topk_indices[i][0].item()].item() + 1
            ...
            # Dynamic Spec
            if self.speculative_config.dynamic_spec:
                ...
                if np.mean(confidence_list) < self.speculative_config.assistant_confidence_threshold:
                    flag = True
